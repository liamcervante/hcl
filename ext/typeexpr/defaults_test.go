...
func TestDefaults_Apply(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"simple object with defaults applied converted": {
			defaults: &Defaults{
				Type: simpleObject,
				DefaultValues: map[string]cty.Value{
					"b": cty.True,
		...
		"tuples convert custom values and dynamic types": {
			defaults: &Defaults{
				Type: cty.List(cty.ObjectWithOptionalAttrs(map[string]cty.Type{
					"name":   cty.String,
					"taints": cty.List(cty.Map(cty.DynamicPseudoType)),
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			if tc.convert {
				var err error
				if got, err = tc.defaults.ApplyAndConvert(tc.value); err != nil {
					t.Errorf("unwanted error: " + err.Error())
					return
				}
			} else {
